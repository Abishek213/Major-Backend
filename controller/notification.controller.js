...
export const getUserNotifications = async (req, res) => {
  try {
    ...
    switch (filter.toLowerCase()) {
      ...
      case "event":
        query.type = {
          $in: ["event_request", "event_response", "event_update"],
        };
        break;
      case "system":
...
export const requestEventNotification = async (req, res) => {
  try {
    ...
    // No need to populate org_ID.fullname – not used
    ...
    res
      .status(201)
      .json(createResponse(true, "Notification sent", notification));
  } catch (error) {
    res
      .status(500)
      .json(
...
export const markAsRead = async (req, res) => {
  try {
    // Ensure notification belongs to the user or their role
    const notification = await Notification.findOneAndUpdate(
      {
        _id: req.params.id,
        $or: [{ userId: req.user._id }, { forRole: req.user.role }],
      },
      { status: "read" },
      { new: true }
    );
    ...
    if (!notification) {
      return res
        .status(404)
        .json(createResponse(false, "Notification not found or unauthorized"));
    }
    ...
    res
      .status(200)
      .json(createResponse(true, "Notification read", { notification }));
  } catch (error) {
    res
      .status(500)
      .json(createResponse(false, "Error marking read", null, error.message));
  }
};
...
export const markAllAsRead = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(
        createResponse(false, "Error marking all read", null, error.message)
      );
  }
};
...
export const approveEventNotification = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(createResponse(true, `Event ${event.status}`, { notification }));
  } catch (error) {
    res
      .status(500)
      .json(
        createResponse(false, "Error processing event", null, error.message)
      );
  }
};
...
export const getAdminNotifications = async (req, res) => {
  try {
    ...
    // Remove the redundant WebSocket broadcast – the HTTP response already contains the data.
    // If real-time updates are needed, they should be triggered when new admin notifications are created.
  } catch (error) {
    res
      .status(500)
      .json(
...
export const getUnreadCount = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(createResponse(true, "Unread count retrieved", response));
  } catch (error) {
    res
      .status(500)
      .json(
        createResponse(false, "Error getting unread count", null, error.message)
      );
  }
};
