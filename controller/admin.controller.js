...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const [userGrowthData, eventGrowthData, requestGrowthData] =
      await Promise.all([
        User.aggregate([
          {
            $match: {
    ...
    const analyticsData = Array.from({ length: 6 }, (_, i) => {
      ...
      const monthUserData = userGrowthData.find(
        (data) =>
          data._id.year === date.getFullYear() &&
          data._id.month === date.getMonth() + 1
      );

      const monthEventData = eventGrowthData.find(
        (data) =>
          data._id.year === date.getFullYear() &&
          data._id.month === date.getMonth() + 1
      );

      const monthRequestData = requestGrowthData.find(
        (data) =>
          data._id.year === date.getFullYear() &&
          data._id.month === date.getMonth() + 1
      );
    ...
    const statsData = [
      {
        ...
        change: calculateGrowth(
          totalUsers,
          totalUsers - (userGrowthData[0]?.count || 0)
        ),
        icon: "Users",
        color: "indigo",
      },
      ...
      },
      {
        ...
        change: calculateGrowth(
          approvedEvents,
          approvedEvents - (eventGrowthData[0]?.upcomingCount || 0)
        ),
        icon: "CheckCircle",
        color: "purple",
      },
      ...
      },
      {
        ...
        value: (
          eventStatusBreakdown.find((status) => status._id === "upcoming")
            ?.count || 0
        ).toString(),
        change: "Scheduled",
        icon: "Clock",
    ...
    res.status(200).json({
      ...
      data: {
        statsData,
        analyticsData,
        usersByRole: userRoleDistribution.reduce(
        ...
        roleStats: {
          distribution: userRoleDistribution.reduce(
            (acc, { roleName, count, recentUsers }) => {
              acc[roleName] = { count, recentUsers };
              return acc;
            },
          ...
          eventStats: roleBasedEventStats.reduce(
...
export const approveRejectEvent = async (req, res) => {
  ...
  try {
    ...
    if (status === "rejected") {
      ...
      if (evtDay.getTime() === nowDay.getTime()) {
        event.status = "ongoing";
      } else if (evtDay < nowDay) {
